...
global.bot = {"id": "305602159741763585", "serverlog":"382001924251320322", "suggestchannel":"382001960984772609", "testingserver":"312667247808217088", "officialserver":"380310916341956610", "prefix":")"};
...
process.on('SIGINT', () => {
  fs.writeFile('./prefixes.json', JSON.stringify(prefixes), 'utf8', (err) => {
    if(err) throw err;
    else { console.log(`Successfully logged ${Object.keys(prefixes).length} prefixes.`); process.exit(); }
  });
});
